...
class WikiSpace(Base, TimestampMixin):
    ...
    read_roles: Mapped[list[str]] = mapped_column(JSON, default=list, comment="可读角色")
    write_roles: Mapped[list[str]] = mapped_column(JSON, default=list, comment="可写角色")
...
class WikiDoc(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    space_id: Mapped[int] = mapped_column(Integer, ForeignKey("wiki_space.id", ondelete="CASCADE"), nullable=False, comment="空间 ID")
    parent_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("wiki_doc.id", ondelete="CASCADE"), nullable=True, comment="父文档 ID")
...
class WikiDocVersion(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    doc_id: Mapped[int] = mapped_column(Integer, ForeignKey("wiki_doc.id", ondelete="CASCADE"), nullable=False, comment="文档 ID")
